...
int main(int argc, char *argv[])
{
    ...
    // int-based row masks
    int* row_has_nz = (int*) std::malloc(jmax * sizeof(int));
    int* active_next= (int*) std::malloc(jmax * sizeof(int));
    ...
    for (int j=0; j<jmax; ++j) {
        int nz = 0;
        const double* r = x[j];
        for (int i=0; i<imax; ++i) { if (r[i] != 0.0) { nz = 1; break; } }
        row_has_nz[j] = nz;
    }   
    ...
    for (int iter = 0; iter < 10000; iter++) {
        ...
        for (int j=1; j<jmax-1; ++j)
            active_next[j] = (row_has_nz[j-1] | row_has_nz[j] | row_has_nz[j+1]);
        active_next[jmax-1] = 0;
